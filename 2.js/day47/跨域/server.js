...
app.use((req, res, next) => {
  ...
  let ss = ary.includes(origin) ? origin:''
	res.header("Access-Control-Allow-Origin", ss);//res.header("Access-Control-Allow-Origin", "*");*是让所有的地址都可以访问，但是不能携带资源凭证（cookies）
	res.header("Access-Control-Allow-Credentials", true);//true在换页面的时候要不要接收
